        ...
        for order in orders:
            if order:
                logging.info('Step {}: {} {}'.format(
                    order.step, order.side, order.quantity))
...
class ManagedRiskActions(TradeActionBase):
    ...
    def __init__(self,
        ...
        self.durations = durations if isinstance(
            durations, list) else [durations]
    ...
    @property
    def action_space(self) -> types.NestedArraySpec:
        if not self._action_space:
            ...
            self.actions = list(
                product(self.portfolio.exchange_pairs, self.actions))
