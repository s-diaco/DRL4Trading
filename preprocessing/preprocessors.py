...
def load_dataset(*, file_name: str) -> pd.DataFrame:
    ...
    _data = pd.read_csv(f"{config['csv']['csv_dir']}/{file_name}")
...
def calculte_price(df):
    ...
    """
    data = df.copy()
    data = data[['datadate', 'tic', 'prccd', 'ajexdi',
...
def preprocess_data():
    ...
    config = configparser.ConfigParser()
    config.read('config.ini')
    df = load_dataset(file_name=config['csv']['training_data_file'])
    # get data after START_DATE
    df = df[df.datadate >= int(config['dates']['start_date'])]
    # calcualte adjusted price
    df_preprocess = calculte_price(df)
...
def calcualte_turbulence(df):
    ...
    config = configparser.ConfigParser()
    config.read('config.ini')
    start = int(config['dates']['calculate_turbulance_after_days'])
